...
typedef enum
{
    ...
    ESTIMATOR_MODE_HALL  = 1              //!< Hall sensor
} ESTIMATOR_Mode_e;
...
#if HALL_BLDC
    // the handle for the Iq PI controller
    PI_Handle	  piHandle_Iq_bldc;

    // the handle for the hall bldc object
    HALL_BLDC_Handle hallBldcHandle;
#endif
...
#if HALL_BLDC
//! \brief Hall structure initialize
extern void HALLBLDC_Init(MOTOR_Handle handle, MotorNum_e motorNum);

//! \brief check hall current state
extern void HALLBLDC_State_Check(MOTOR_Handle handle, MotorNum_e motorNum);

//! \brief check hall current state
extern void HALLBLDC_Ctrl_Run(MOTOR_Handle handle);
#endif
