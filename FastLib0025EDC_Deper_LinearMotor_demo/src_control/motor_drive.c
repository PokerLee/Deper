...
#if HALL_BLDC
CODE_SECTION(".data") 		   HALL_BLDC_Obj 	 hallBldc_M1;
CODE_SECTION(".data") 		   HALL_BLDC_Obj 	 hallBldc_M2;
CODE_SECTION(".data") 		   PI_Obj 			 pi_Iq_bldc_M1;
CODE_SECTION(".data") 		   PI_Obj 			 pi_Iq_bldc_M2;
#endif
#if HALL_FAST
CODE_SECTION(".data") 		   HALL_Obj 		 hall_M1;
CODE_SECTION(".data") 		   HALL_Obj 		 hall_M2;
#if (USER_MOTOR1 == my_pm_motor_1)
// 24V Motor
const float32_t hallAngleBuf[7] = { 0.5736369f,  -2.6209536f, 1.6240141f, \
									2.6195664f, -0.48745465f , -1.5250618f,
									0.5736369f };

//const float32_t hallAngleBuf[7] = { 2.649765,  -0.46309674, 1.6316903, \
//									0.5553597, -2.6144533 , -1.4993204,
//									2.649765 };

#endif  // USER_MOTOR1 == my_pm_motor_1
#endif  // HALL_FAST
...
void initMotorCtrlParameters(MOTOR_Handle handle, const MotorNum_e motorNum)
{
	...
	if(motorNum == MTR_1)
	{
	    ...
	    HALL_setGPIOs(obj->hallHandle, MTR1_HALLBLDC_A_GPIO, MTR1_HALLBLDC_B_GPIO, MTR1_HALLBLDC_C_GPIO);
...
//		HAL_setupMtrFaults(obj->halMtrHandle, motorNum);
...
#if HALL_BLDC
		obj->flagModeSelect = 1;
		obj->hallBldcHandle = &hallBldc_M1;
		obj->piHandle_Iq_bldc = &pi_Iq_bldc_M1;

		// Sets up #if HALL_BLDC
		HALLBLDC_Init(handle, motorNum);
#else // FAST
		obj->flagModeSelect = 0;
#endif
	}
	...
	if(motorNum == MTR_2)
	{
	    ...
	    // initialize the hall handle
	    obj->hallHandle = HALL_init(&hall_M2, sizeof(hall_M2));

	    // set the HALL controller parameters
	    HALL_setParams(obj->hallHandle, obj->userParamsHandle);
	    HALL_setAngleBuf(obj->hallHandle, &hallAngleBuf[0]);
	    HALL_setGPIOs(obj->hallHandle, MTR2_HALLBLDC_A_GPIO, MTR2_HALLBLDC_B_GPIO, MTR2_HALLBLDC_C_GPIO);
...
#if HALL_BLDC
		obj->flagModeSelect = 1;
		obj->hallBldcHandle = &hallBldc_M2;
		obj->piHandle_Iq_bldc = &pi_Iq_bldc_M2;
	...
	else if(motorNum == MTR_2)
	{
		...
		if(obj->flagEnableOffsetCalc == true)
		{
			...
			for(offsetCnt = 0; offsetCnt < 32000; offsetCnt++)
			{
				...
				if(motorNum == MTR_1)
				{
					HAL_readMtrADCData(&obj->adcData, MTR_1);
				}
				else if(motorNum == MTR_2)
				{
					HAL_readMtrADCData(&obj->adcData, MTR_2);
				}
...
//float32_t debug[200];
//uint32_t debugCount;
__interrupt CODE_SECTION( "ramfuncs") void motorCtrlISR(void)
{
    ...
    uint8_t i =0;
    for(i = MTR_1; i <= MTR_2; i++)
    {
    	// 读取相电流
    	HAL_readMtrADCData(&obj[isrNum]->adcData, isrNum);
		...
		// 相电压零点偏差矫正
		obj[isrNum]->adcData.V_V.value[0] -= obj[isrNum]->adcData.offset_V_sf.value[0] * obj[isrNum]->adcData.VdcBus_V;
		obj[isrNum]->adcData.V_V.value[1] -= obj[isrNum]->adcData.offset_V_sf.value[1] * obj[isrNum]->adcData.VdcBus_V;
		obj[isrNum]->adcData.V_V.value[2] -= obj[isrNum]->adcData.offset_V_sf.value[2] * obj[isrNum]->adcData.VdcBus_V;
    ...
    for(i = MTR_1; i <= MTR_2; i++)
    {
...
#if HALL_BLDC
		// HALL扇区判断与速度计算
    	HALLBLDC_State_Check(obj[isrNum],isrNum);
#endif
		// 相电流Clark变换
        CLARKE_run(obj[isrNum]->clarkeHandle_V,&obj[isrNum]->adcData.V_V, &obj[isrNum]->estInputData.Vab_V);
        ...
        // 常规运行模式
        else if(obj[isrNum]->flagMotorIdentified == true)
        {
        ...
        // 计算电角度并角度补偿
        obj[isrNum]->angleDelta_rad = objUser[isrNum]->angleDelayed_sf_sec * obj[isrNum]->estOutputData.fm_lp_rps;
        obj[isrNum]->angleEST_rad = MATH_incrAngle(obj[isrNum]->estOutputData.angle_rad, obj[isrNum]->angleDelta_rad);
...
#if HALL_FAST
		HALL_setTimeStamp(obj[isrNum]->hallHandle, HAL_calcCAPCount(obj[isrNum]->halMtrHandle));
		HALL_run(obj[isrNum]->hallHandle, obj[isrNum]->speed_int_Hz);
		obj[isrNum]->angleHall_rad = HALL_getAngle_rad(obj[isrNum]->hallHandle);
		obj[isrNum]->speedHall_Hz = HALL_getSpeed_Hz(obj[isrNum]->hallHandle);

		// HALL_CAL
//		HALL_calibrateIndexAngle(obj[isrNum]->hallHandle, obj[isrNum]->angleEST_rad);
#endif

        // 电机状态机判断
        // 全速运行模式
	    if(obj[isrNum]->estimatorMode == ESTIMATOR_MODE_FAST)
	    {
			...
			else if(obj[isrNum]->motorState == MOTOR_ALIGNMENT)
			{
				...
				// 交轴电流Iq给定位0，Id给定为对齐电流alignCurrent_A
		...
		{
//			obj[isrNum]->speed_Hz = obj[isrNum]->speedHall_Hz;

			if(obj[isrNum]->motorState >= MOTOR_CL_RUNNING)
			{
				obj[isrNum]->angleFOC_rad = obj[isrNum]->angleHall_rad;
			}
			else if(obj[isrNum]->motorState == MOTOR_OL_START)
			{
				obj[isrNum]->angleFOC_rad = obj[isrNum]->angleHall_rad;
				...
				{
					obj[isrNum]->IsRef_A = obj[isrNum]->startCurrent_A;
					obj[isrNum]->Idq_out_A.value[1] = obj[isrNum]->startCurrent_A;
				}
				else
				{
					obj[isrNum]->IsRef_A = -obj[isrNum]->startCurrent_A;
				...
				obj[isrNum]->motorState = MOTOR_CL_RUNNING;
				PI_setUi(obj[isrNum]->piHandle_spd, (obj[isrNum]->frswPos_sf * obj[isrNum]->Idq_out_A.value[1]));
			}
			else if(obj[isrNum]->motorState == MOTOR_ALIGNMENT)
			{
				obj[isrNum]->angleFOC_rad = 0.0f;
				...
				TRAJ_setIntValue(obj[isrNum]->trajHandle_spd, 0.0f);
				EST_setAngle_rad(obj[isrNum]->estHandle, obj[isrNum]->angleFOC_rad);
				HALL_setForceAngleAndIndex(obj[isrNum]->hallHandle, obj[isrNum]->speedRef_Hz);
        ...
        // 给定Park变换结构体中电角度sin/cos值
        PARK_setPhasor(obj[isrNum]->parkHandle_I, &phasor);

        // 启用相电流Park变换
        PARK_run(obj[isrNum]->parkHandle_I, &(obj[isrNum]->estInputData.Iab_A),(MATH_vec2 *)&(obj[isrNum]->Idq_in_A));
        ...
        {
            // 最大电压输出
            objUser[isrNum]->maxVsMag_V = objUser[isrNum]->maxVsMag_pu * obj[isrNum]->adcData.VdcBus_V;

            PI_setMinMax(obj[isrNum]->piHandle_Id, -objUser[isrNum]->maxVsMag_V, objUser[isrNum]->maxVsMag_V);
        ...
        // 运行反Park变换
        IPARK_run(obj[isrNum]->iparkHandle_V, &obj[isrNum]->Vdq_out_V, &obj[isrNum]->Vab_out_V);

        // SVPWM输入设定
        SVGEN_setup(obj[isrNum]->svgenHandle, obj[isrNum]->oneOverDcBus_invV);
        ...
        // FAST
        if(obj[isrNum]->flagModeSelect == 0)
        {
        	// do nothing
        }
...
#if HALL_BLDC
        // HALL or HALL + FAST
        else if((obj[isrNum]->flagModeSelect == 1) ||(obj[isrNum]->flagModeSelect == 2))
        {
        	if(obj[isrNum]->flagRunIdentAndOnLine == true)
        	{
        		HALLBLDC_Ctrl_Run(obj[isrNum]);
        	}
        }
#endif
        // PWM比较值设定
...
//    // DEBUG
//    debug[debugCount] = obj[0]->hallBldcHandle->Hall_GpioData;
//    debugCount++;
//    if(debugCount>=200)
//    {debugCount = 0;}
